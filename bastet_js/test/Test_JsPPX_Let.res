...
describe("PPX_Let", () => {
  ...
  describe("bind", () => {
    ...
    it(
      "should resolve first callback first when it resolves faster",
      done_ => bind_both(fast_cb, slow_cb, \"<."(done_, expect_first(to_be_below))),
    )
    it(
      "should resolve first callback first when it resolves slower",
      done_ => bind_both(slow_cb, fast_cb, \"<."(done_, expect_first(to_be_below))),
    )
  })
  describe("both", () => {
    it(
      "should resolve first callback first when it resolves faster",
      done_ => both(fast_cb, slow_cb, \"<."(done_, expect_first(to_be_below))),
    )
    it(
      "should resolve first callback second when it resolves slower",
      done_ => both(slow_cb, fast_cb, \"<."(done_, expect_first(to_be_above))),
    )
  })
})
